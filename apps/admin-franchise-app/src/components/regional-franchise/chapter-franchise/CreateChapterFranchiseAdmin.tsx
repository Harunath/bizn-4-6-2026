...
export default function CreateChapterFranchiseAdmin({
	chapterId,
}: {
	chapterId: string;
}) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const res = await fetch(
				`/api/regional-franchise/chapter-franchise/${chapterId}`,
				{
					method: "POST",
	...
	return (
		<form
			...
			className="max-w-xl mx-auto p-8 bg-neutral-100 text-neutral-900 rounded-xl shadow-2xl space-y-6">
			<h2 className="text-3xl font-semibold text-center text-gray-800">
				Create Chapter Franchise Admin
			</h2>
